...
int main(int argc, char** argv) {
  ...
  if(argc < 4) {
      std::cout << "Usage: ./mpi_ex_2d <datasize> <gpus/resource_set> <proccount>" << std::endl;
      return 0;
  }
  ...
  for (int i = 0; i < procdim; i++) {
    ...
    for (int j = 0; j < trials; j++) {
      MPICHECK(MPI_Bcast((void *)sendbuff, n, MPI_DOUBLE, i, mpi_row_comm));
    }
    double row_time_end = MPI_Wtime();
    ...
    double col_time_end = MPI_Wtime();
    col_time += col_time_end - col_time_start;
  }
...
#ifdef CUDA_AWARE
  std::cout << "rank: " << rank << " size: " << (n * sizeof(double)) << " gpu_time: " << total_time << " bw: " << ((2 * procdim * n * sizeof(double) * trials) / total_time) << std::endl;
#else
  std::cout << "rank: " << rank << " size: " << (n * sizeof(double)) << " cpu_time: " << total_time << " bw: " << ((2 * procdim * n * sizeof(double) * trials) / total_time) << std::endl;
#endif
